...
1）基于TCP/IP协议：TCP/IP协议相当于两个人交流时确定的语言为汉语，http协议相当于确定用汉语的古文交流。

 2）基于请求-响应模式：和websoket不同，比如聊天室，服务器可以先给客户端通讯，但是http协议只能先客户端（浏览器）给服务器发，然后服务器在响应。

3）无状态保存：服务器不对访问做任何记录。即客户端给服务器发一次请求，服务器响应这次请求时，服务器不知道这次请求之前客户端给服务器发了多少请求，发了几次请求等。比如说登陆的功能，基于http就做不到，因为服务器要记住之前登录成功过。

### 1.1 http的请求、响应格式

 ![image-20250123120053590](C:\Users\26344\AppData\Roaming\Typora\typora-user-images\image-20250123120053590.png)

一个完整的URL包括：**协议，ip，端口，路径，参数**。

例如：https://www.baidu.com/s?wd=yuan	其中https时协议，www.baidu.com是ip（域名转为ip），端口默认80，/s是路径，参数是wd=yuan。

请求方式：get请求和post请求

- GET提交的数据会放在URL之后，？分割URL和传输数据，参数之间&相连。
- GET提交的数据大小有限制，因为浏览器对URL长度有限制，而POST方法提交的数据没有限制。

状态响应码：例如 200 OK，3位数字和原因组成。404也是返回的状态响应码。

​	自己手写一个服务器绑定在本机的8080端口，然后用浏览器输入127.0.0.1:8080来访问自己的服务器：

```python
import socket

sock = socket.socket() #套接字：sock

sock.bind(('127.0.0.1',8080))
sock.listen(5) #sock这里作为服务器
while 1:
    conn,addr = sock.accept() #等待客户端连接，阻塞等待
    #conn是服务端和客户端通信的双向管道
    data = conn.recv(1024)
    print('客户端发送的数据请求信息\n:',data)

    conn.send(b'HTTP/1.1 200 OK\r\nsever:wangbx\r\ncontent-type:text/html\r\n\r\n<h1>hello world xiangyingti</h1>')
    conn.close()
```

平常开发大型Web应用程序直接用socket不就完了？可以做但是，会很麻烦。Web框架比如fastapi会把很多功能封装好，会完成比如说拆解请求信息，得到所有请求头等等。

# 2.fastapi

- **uvicorn本质上封装了一个socket**，加在web应用程序（或者是fastapi框架）的前面，做一些接收等动作。下面这个案例比自己写socket简单多了。
- ![image-20250214091014958](C:\Users\26344\AppData\Roaming\Typora\typora-user-images\image-20250214091014958.png)

```python
from fastapi import FastAPI

app = FastAPI() # FASTAPI是一个python类

@app.get("/")
async def home():
    return {"message":"hello wangbixiong"} # fastapi会自动封装格式
@app.get("/shop")
async def shop():
    return {"shop":"商品信息"} # fastapi会自动封装格式
```

@app.get("/shop")的意思是用户输入./shop路径的话，则执行被装饰器修饰的的函数。

- **restful**：  用POST、DELETE、UPDATE等请求动作来表达对数据的增删查改。（GET一般查看，POST一般添加）
...
![[外链图片转存失败,源站可能有防盗链机制,建议将图片保存下来直接上传(img-vCc4wgdh-1689471141827)(assets/starlette.png)]](https://i-blog.csdnimg.cn/blog_migrate/4db36384fd0f262d7fe328ead2bf3b73.png)
...
![[外链图片转存失败,源站可能有防盗链机制,建议将图片保存下来直接上传(img-eOlHUxXV-1689471141827)(assets/%E6%88%AA%E5%B1%8F2022-08-28%2020.18.31.png)]](https://i-blog.csdnimg.cn/blog_migrate/2f3613498472f3c3fd602043a7606cb3.png)
...
![[外链图片转存失败,源站可能有防盗链机制,建议将图片保存下来直接上传(img-YeQtObnw-1689471141827)(assets/image-20220901214542004.png)]](https://i-blog.csdnimg.cn/blog_migrate/f0c7485bdd0caa62a75c8bcbab1e592d.png)
...
![[外链图片转存失败,源站可能有防盗链机制,建议将图片保存下来直接上传(img-wB8IvpyJ-1689471141828)(assets/depended_frontend_backend.png)]](https://i-blog.csdnimg.cn/blog_migrate/446294901be2bc2e60202c8acfa4eb56.png)

1. 前后端分离【把前端的界面效果(html，css，js分离到另一个服务端，python服务端只需要返回数据即可)】

前端形成一个独立的网站，服务端构成一个独立的网站

![[外链图片转存失败,源站可能有防盗链机制,建议将图片保存下来直接上传(img