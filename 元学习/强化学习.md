...
| Object（主要学什么）                                   | 祖公式 / 核心恒等式                                                                                                                                                                                                                                                       | 估计器（训练信号从哪来）                                                                                                                          | 稳定器（典型防崩机制）                                                            | 典型代表                          |
| ----------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------- | ----------------------------- |
| Policy $\pi_\theta$（随机策略）                       | $$\nabla_\theta J(\theta)=\mathbb{E}_{s\sim d^{\pi_\theta},,a\sim \pi_\theta}\left[\nabla_\theta \log \pi_\theta(a\mid s)Q^{\pi_\theta}(s,a)\right]$$$$=\mathbb{E}\left[\nabla_\theta \log \pi_\theta(a\mid s)A^{\pi_\theta}(s,a)\right]$$                        | return-to-go：$$G_t=\sum_{k=0}^{T-t}\gamma^k r_{t+k}$$baseline/优势：$$\hat A_t=G_t-b(s_t)\quad\text{或}\quad \hat A_t=Q(s_t,a_t)-V(s_t)$$ | KL/信任域/裁剪（限制新旧策略差异）                                                    | REINFORCE / TRPO / PPO        |
| Actor-Critic $\pi_\theta$ + $V_\phi$ 或 $Q_\phi$ | Actor：$$\nabla_\theta J(\theta)=\mathbb{E}\left[\nabla_\theta\log \pi_\theta(a_t\mid s_t)\hat A_t\right]$$Critic（TD 误差）：$$\delta_t=r_t+\gamma V_\phi(s_{t+1})-V_\phi(s_t)$$                                                                                       | 最简单：$$\hat A_t=\delta_t$$GAE：$$\hat A_t^{\text{GAE}(\gamma,\lambda)}=\sum_{l=0}^{\infty}(\gamma\lambda)^l,\delta_{t+l}$$              | on-policy：PPO clip / KLoff-policy：target/Polyak、replay、双Q/min-double-Q | A2C/A3C、PPO、DDPG/TD3、SAC      |
| Value / $Q^\pi$（评估给定策略）                         | Bellman 期望方程：$$V^\pi(s)=\mathbb{E}_{a\sim\pi,,s'\sim P}\left[r(s,a)+\gamma V^\pi(s')\right]$$$$Q^\pi(s,a)=\mathbb{E}_{s'\sim P,,a'\sim \pi}\left[r(s,a)+\gamma Q^\pi(s',a')\right]$$                                                                              | TD(0) target：$$y_t=r_t+\gamma V(s_{t+1})$$n-step：$$y_t=\sum_{k=0}^{n-1}\gamma^k r_{t+k}+\gamma^n V(s_{t+n})$$                         | target network / 归一化 / 稳定的 bootstrap                                   | TD、SARSA、各种 Critic 训练         |
| Optimal $Q^*$（最优控制）                             | Bellman最优方程：$$Q^*(s,a) = \mathbb{E}\left[r(s,a) + \gamma \max_{a'} Q^*(s',a')\right]$$                                                                                                                                                                            | Q-target（DQN 族核心）：$$y=r+\gamma \max_{a'}Q_{\text{targ}}(s',a')$$                                                                      | replay + target network；Double-Q / min-double-Q 抗过估计                   | Q-learning、DQN、Double DQN、TD3 |
| Q-learning（控制的随机近似更新）                           | $$Q(s_t,a_t)\leftarrow Q(s_t,a_t)+\alpha\left(r_t+\gamma \max_{a'}Q(s_{t+1},a')-Q(s_t,a_t)\right)$$                                                                                                                                                               | bootstrap target 就是上式括号里的 TD 误差                                                                                                       | 经验回放、target network（深度版本）                                              | Q-learning、DQN                |
| 正则化目标（KL / 熵）                                   | KL 正则（RLHF 常见）：$$\max_\pi\mathbb{E}[R]-\beta\mathbb{E}_{s}\left[\mathrm{KL}\left(\pi(\cdot\mid s),\|,\pi_{\mathrm{ref}}(\cdot\mid s)\right)\right]$$最大熵：$$\max_\pi;\mathbb{E}\left[\sum_t\gamma^t\left(r_t+\alpha,\mathcal{H}(\pi(\cdot\mid s_t))\right)\right]$$ | KL/熵项如何估：token-level vs sequence-level、组内归一化等                                                                                         | KL penalty/constraint、clip、温度/门控（soft gate）                            | PPO-RLHF、SAC、GSPO/SAPO 等      |
| Off-policy 修正（分布错配）                             | 重要性采样比率：$$\rho_t=\frac{\pi(a_t\mid s_t)}{\mu(a_t\mid s_t)}$$                                                                                                                                                                                                      | 截断比率：$$\bar\rho_t=\min(\bar\rho,\rho_t)$$（或 V-trace/Retrace 一类的修正）                                                                    | 截断/控制方差；actor-learner 解耦时常用 V-trace                                    | IMPALA、ACER 等                 |
| Offline RL（固定数据集）                               | CQL（离散动作常见写法之一）：$$\min_Q;\mathbb{E}_{(s,a,r,s')\sim D}\left[(Q(s,a)-\mathcal{B}Q(s,a))^2\right]+\alpha\left(\mathbb{E}_{s\sim D}\left[\log\sum_a \exp(Q(s,a))\right]-\mathbb{E}_{(s,a)\sim D}[Q(s,a)]\right)$$                                                    | $\mathcal{B}Q$ 是 Bellman backup（用数据集近似）                                                                                               | “保守项”抑制 OOD 动作的 Q 过估计                                                  | CQL、IQL、BCQ 等                 |
| Model $\hat P,\hat r$（模型派）                      | 动力学最大似然：$$\max_\psi;\mathbb{E}_{(s,a,s')\sim D}\left[\log \hat P_\psi(s'\mid s,a)\right]$$奖励回归（例）：$$\min_\psi;\mathbb{E}\left[(\hat r_\psi(s,a)-r)^2\right]$$                                                                                                     | one-step / short rollout（短想象更稳）                                                                                                       | MPC 重规划、短 rollout、ensemble/不确定性                                        | Dyna、MBPO、world model 线       |
| Distributional RL（回报分布 (Z)）                     | 分布式贝尔曼：$$Z^\pi(s,a)\overset{D}{=}R(s,a)+\gamma Z^\pi(s',a')$$期望关系：$$Q^\pi(s,a)=\mathbb{E}[Z^\pi(s,a)]$$                                                                                                                                                           | 分布匹配：C51（分类）、QR（分位数回归）等                                                                                                               | 投影/分位数回归的稳定实现                                                          | C51、QR-DQN、Rainbow 组件         |
| Occupancy measure $\rho_\pi$（占用度量）              | 折扣占用度量：$$\rho_\pi(s,a)=(1-\gamma)\sum_{t=0}^{\infty}\gamma^t,\Pr_\pi(s_t=s,;a_t=a)$$                                                                                                                                                                              | 用 $\rho_\pi$ 把 RL 写成对分布的优化（常见于 IRL/约束RL）                                                                                              | 约束/对偶（拉格朗日）等                                                           | IRL/GAIL/约束RL 的常用视角           |
| Preference（不一定 rollout 的“对齐优化”）                 | DPO（常见等价写法之一）：$$\mathcal{L}_{\mathrm{DPO}}(\theta)=-\mathbb{E}\left[\log \sigma\left(\beta\left(\log\frac{\pi_\theta(y^+\mid x)}{\pi_{\mathrm{ref}}(y^+\mid x)}-\log\frac{\pi_\theta(y^-\mid x)}{\pi_{\mathrm{ref}}(y^-\mid x)}\right)\right)\right]$$            | 直接用偏好对 $(y^+,y^-)$ 构造分类式目标                                                                                                            | 不需要在线 rollout（训练更稳更省）                                                  | DPO、以及一批 DPO-like             |
