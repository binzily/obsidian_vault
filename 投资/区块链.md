...
挖矿实际上求hash只用到了block header（block header里面包括nounce，挖矿会不断调整这个nounce以及coinbase parameter）里面的内容，交易的具体信息在block header里面是没有的，block header里面只有一个 merkle trea 的根hash值就够了。![|700](assets/区块链/file-20260228115404327.png)
