...
off-policy 的经典修法是重要性采样：如果要估计新分布下的期望，你得用 $\frac{\pi_\theta}{\pi_{\text{old}}}$ 去重加权。但关键在于——语言生成是“序列动作”，奖励通常也是序列级别的（整段答案对/不对、整体偏好分数等）。这时理论上对应的是序列级重要性比率 $$r_{\text{seq}}(x,y) = \frac{\pi_\theta(y|x)}{\pi_{\text{old}}(y|x)} = \prod_{t=1}^T \frac{\pi_\theta(y_t|x,y_{<t})}{\pi_{\text{old}}(y_t|x,y_{<t})}$$
