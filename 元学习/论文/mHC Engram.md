...
把它想成：模型除了“算”（attention/MLP）之外，多了一个“**查**”（lookup）的原语；查到的是一组**静态嵌入向量**，再用当前上下文对这些向量做“动态融合”。**表就是模型参数**，通常随机初始化，然后在预训练中端到端用梯度学出来；不是人工统计出来的词频表，所以EnGram和训练好的模型是绑到一起的。
...
---
### 4.2 作为工程师：通常“用带 Engram 的模型”，而不是自己加
