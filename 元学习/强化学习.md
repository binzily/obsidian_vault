...
3) 为什么 $w(\tau)$ 会变成“连乘的 ratio”（而且环境转移会抵消） 
       轨迹分布可写成（环境转移不依赖 $\theta$）： $$p_\theta(\tau) = \rho_0(s_0) \prod_{t=0}^T \pi_\theta(a_t | s_t) P(s_{t+1} | s_t,a_t)$$因此比值： $$\frac{p_\theta(\tau)}{p_{\text{old}}(\tau)} = \prod_{t=0}^T \frac{\pi_\theta(a_t | s_t)}{\pi_{\text{old}}(a_t | s_t)}$$所有 $P(\cdot)$ 都消掉了，所以 IS 权重只依赖两套策略，不依赖环境动态。Sutton & Barto 在讲 off-policy MC 时把这个“转移概率抵消”写得很明确，并给出了乘积形式的 ratio（他们用 $\mu$ 表示行为策略/旧策略）。 映射到  LLM: $s_t = (x,y_{<t})$，$a_t = y_t$，于是 $$w_{\text{seq}}(y) = \prod_t \frac{\pi_\theta(y_t | x,y_{<t})}{\pi_{\text{old}}(y_t | x,y_{<t})}$$这就是你一直写的“序列级重要性比率”。
4) 那 TRPO/PPO 里的 ratio 是怎么“冒出来”的？（它其实是 IS 的一个“局部用法”） 
       TRPO 的推导里有一个关键步骤：目标里会出现 $\sum_a \pi_\theta(a | s) A_{\text{old}}(s,a)$ 这种“对动作求和”的项，但你手上只有用某个采样分布 $q(a | s)$（通常就是旧策略）抽到的动作样本。于是把“对动作求和”改写成 重要性采样估计： $$\sum_a \pi_\theta(a | s)A(s,a) = \mathbb{E}_{a\sim q(\cdot|s)}\left[\frac{\pi_\theta(a | s)}{q(a | s)}A(s,a)\right]$$TRPO 论文在“replace the sum over the actions by an importance sampling estimator”处直接写了这一点，并且后面说明 single-path 里取 $q(a | s) = \pi_{\theta_{\text{old}}}(a | s)$。 PPO 论文把 TRPO 的 surrogate 直接写成（他们记作 $\theta_{\text{old}}$）： $$\max_\theta \hat{\mathbb{E}}_t\left[\frac{\pi_\theta(a_t | s_t)}{\pi_{\theta_{\text{old}}}(a_t | s_t)} \hat{A}_t\right]$$并定义 $r_t(\theta) = \frac{\pi_\theta(a_t | s_t)}{\pi_{\theta_{\text{old}}}(a_t | s_t)}$。 所以：PPO/GRPO 里的 $r_t$ 形式上就是 IS ratio，只不过它是用在“每个状态 $s_t$ 条件下的动作分布”这个层面（而不是整条轨迹的 $w(\tau)$）。
