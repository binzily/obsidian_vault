...
>"分布校正"：你手里有的数据是按 **旧分布/旧策略** $\pi_{\text{old}}$​ 采出来的，但你想优化的是 **新分布/新策略** $\pi_\theta$。重要性采样的做法是：给每个采样到的动作/样本一个权重$w = \frac{\pi_\theta(\text{sample})}{\pi_{\text{old}}(\text{sample})}$ ，然后用 **加权后的样本** 去近似“如果这些样本是从 $\pi_\theta$ 来的会怎样”。这就叫“**用权重把分布从 old 校正到 new**”。
>
>"单一样本做分布校正"：在每个 token 位置只采到一个 token，就试图用它的概率比率去代表整个位点的分布差异；理论上需要大量样本平均才稳定，现实里会变成高方差噪声。
>在 LLM-RL（GRPO 那套）里，每个 token 位置 t 的“动作”就是生成的那个 $token a_t$​。但在某个前缀状态 $s_t$ 下，你通常只采到 **一个** token（一次只走了一条路径）。于是你用 **这一条路径上这一个 token** 的比率$r_t(\theta) = \frac{\pi_\theta(a_t | s_t)}{\pi_{\text{old}}(a_t | s_t)}$来做“分布校正”。
