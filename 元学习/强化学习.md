...
4) TRPO/PPO 的目标是：**不要用这个高方差的轨迹级 $w(\tau)$**，改用更稳定的“局部近似 + 信赖域”。
       TRPO 先定义策略表现（折扣回报）$\eta(\pi)$，以及 $Q^\pi, V^\pi, A^\pi$（优势） 然后给出一个关键恒等式（Kakade & Langford）： $$\eta(\tilde{\pi}) = \eta(\pi) + \mathbb{E}_{\tau\sim\tilde{\pi}}\left[\sum_{t\ge0} \gamma^t A^\pi(s_t,a_t)\right]$$
       直觉：**新策略的期望回报 = 旧策略期望回报 + 在新策略采样到的轨迹上，旧策略优势函数的折扣和**。
       并进一步“把沿时间步求和”改写成“按状态出现次数加权”： $$\eta(\tilde{\pi}) = \eta(\pi) + \sum_s \rho_{\tilde{\pi}}(s) \sum_a \tilde{\pi}(a|s) A^\pi(s,a)$$符号解释： 
    - $\tilde{\pi}$：你想更新得到的新策略 
    - $\pi$：当前/旧策略（后面常写 $\pi_{\text{old}}$）
    - $\rho_{\tilde{\pi}}(s)$：折扣访问频率（discounted visitation frequency），直观就是“在 $\tilde{\pi}$ 下状态 $s$ 被访问到的加权次数” 
