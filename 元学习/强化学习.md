...
>在 LLM-RL（GRPO 那套）里，每个 token 位置 t 的“动作”就是生成的那个 $token \ a_t$​。但在某个前缀状态 $s_t$ 下，你通常只采到 **一个** token（一次只走了一条路径）。于是你用 **这一条路径上这一个 token** 的比率$r_t(\theta) = \frac{\pi_\theta(a_t | s_t)}{\pi_{\text{old}}(a_t | s_t)}$来做“分布校正”，**只采样一次就用重要性采样本身就会带来偏差。**
