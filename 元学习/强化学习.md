...
## 5.强化学习算法大一统
| Object（主要学什么）                         | 祖公式/不变量（核心方程）                                                                                                                                                                                                                                                                                                                                                                                        | 估计器（训练信号怎么造）                                                                                                                                                                                                                                               | 稳定器（常见防崩）                                                                                                                                                                                                              | 代表算法                                                                                                                                                                                                                   |
| ------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Policy ($\pi_\theta$)**（纯策略梯度）      | $\nabla_\theta J(\theta)=\mathbb{E}_\tau!\left[\sum_{t=0}^{T}\nabla_\theta \log \pi_\theta(a_t\mid s_t),G_t\right]$                                                                                                                                                                                                                                                                                  | $G_t=\sum_{k=t}^{T}\gamma^{k-t}r_k$（return-to-go）; baseline $b(s_t)$ 降方差                                                                                                                                                                                   | 熵正则、KL penalty；或直接引入信任域（见 TRPO/PPO 行）                                                                                                                                                                                  | REINFORCE                                                                                                                                                                                                              |
| **Actor-Critic**（(\pi)+(V/Q)）         | **Actor**：(\nabla_\theta J=\mathbb{E}!\left[\nabla_\theta \log \pi_\theta(a\mid s),A^\pi(s,a)\right])；**Critic**：(V^\pi(s)=\mathbb{E}[r+\gamma V^\pi(s')]) ([NeurIPS Papers](https://papers.neurips.cc/paper/1713-policy-gradient-methods-for-reinforcement-learning-with-function-approximation.pdf?utm_source=chatgpt.com "Policy Gradient Methods for Reinforcement Learning with Function ...")) | TD 误差 (\delta_t=r_t+\gamma V(s_{t+1})-V(s_t))；或 GAE（见 GAE 行） ([Stanford University](https://web.stanford.edu/class/psych209/Readings/SuttonBartoIPRLBook2ndEd.pdf?utm_source=chatgpt.com "Reinforcement Learning: An Introduction - Stanford University")) | on-policy：PPO clip / KL；off-policy：target/Polyak、双Q、replay ([arXiv](https://arxiv.org/abs/1707.06347?utm_source=chatgpt.com "Proximal Policy Optimization Algorithms"))                                                | A2C/A3C、PPO、SAC ([arXiv](https://arxiv.org/abs/1707.06347?utm_source=chatgpt.com "Proximal Policy Optimization Algorithms"))                                                                                           |
| **Value (V^\pi)**（策略评估）               | (V^\pi(s)=\mathbb{E}_{a\sim\pi(\cdot\mid s),,s'\sim P(\cdot\mid s,a)}!\left[r(s,a)+\gamma V^\pi(s')\right]) ([Stanford University](https://web.stanford.edu/class/psych209/Readings/SuttonBartoIPRLBook2ndEd.pdf?utm_source=chatgpt.com "Reinforcement Learning: An Introduction - Stanford University"))                                                                                            | TD(0)、n-step、(\lambda)（偏差-方差权衡） ([Stanford University](https://web.stanford.edu/class/psych209/Readings/SuttonBartoIPRLBook2ndEd.pdf?utm_source=chatgpt.com "Reinforcement Learning: An Introduction - Stanford University"))                              | target network / 归一化 / replay（深度函数逼近下更稳） ([谷歌云存储](https://storage.googleapis.com/deepmind-media/dqn/DQNNaturePaper.pdf?utm_source=chatgpt.com "Human-level control through deep reinforcement learning"))              | TD、critic 训练 ([Stanford University](https://web.stanford.edu/class/psych209/Readings/SuttonBartoIPRLBook2ndEd.pdf?utm_source=chatgpt.com "Reinforcement Learning: An Introduction - Stanford University"))             |
| **Action-Value (Q^\pi)**（策略评估）        | (Q^\pi(s,a)=\mathbb{E}_{s'\sim P(\cdot\mid s,a),,a'\sim\pi(\cdot\mid s')}!\left[r(s,a)+\gamma Q^\pi(s',a')\right]) ([Stanford University](https://web.stanford.edu/class/psych209/Readings/SuttonBartoIPRLBook2ndEd.pdf?utm_source=chatgpt.com "Reinforcement Learning: An Introduction - Stanford University"))                                                                                     | TD target：(y=r+\gamma Q(s',a'))（其中 (a'\sim\pi) 或来自采样） ([Stanford University](https://web.stanford.edu/class/psych209/Readings/SuttonBartoIPRLBook2ndEd.pdf?utm_source=chatgpt.com "Reinforcement Learning: An Introduction - Stanford University"))        | target network、replay ([谷歌云存储](https://storage.googleapis.com/deepmind-media/dqn/DQNNaturePaper.pdf?utm_source=chatgpt.com "Human-level control through deep reinforcement learning"))                                 | SARSA、各类 Q-critic ([Stanford University](https://web.stanford.edu/class/psych209/Readings/SuttonBartoIPRLBook2ndEd.pdf?utm_source=chatgpt.com "Reinforcement Learning: An Introduction - Stanford University"))        |
| **Optimal (Q^*)**（最优控制/价值派主线）         | (Q^_(s,a)=\mathbb{E}_{s'\sim P(\cdot\mid s,a)}!\left[r(s,a)+\gamma \max_{a'}Q^_(s',a')\right]) ([Stanford University](https://web.stanford.edu/class/psych209/Readings/SuttonBartoIPRLBook2ndEd.pdf?utm_source=chatgpt.com "Reinforcement Learning: An Introduction - Stanford University"))                                                                                                         | bootstrap target：(y=r+\gamma \max_{a'}Q_{\text{tgt}}(s',a')) ([谷歌云存储](https://storage.googleapis.com/deepmind-media/dqn/DQNNaturePaper.pdf?utm_source=chatgpt.com "Human-level control through deep reinforcement learning"))                              | replay + target network（DQN 核心稳态） ([谷歌云存储](https://storage.googleapis.com/deepmind-media/dqn/DQNNaturePaper.pdf?utm_source=chatgpt.com "Human-level control through deep reinforcement learning"))                     | Q-learning、DQN ([谷歌云存储](https://storage.googleapis.com/deepmind-media/dqn/DQNNaturePaper.pdf?utm_source=chatgpt.com "Human-level control through deep reinforcement learning"))                                        |
| **GAE（优势估计器）**（AC/PPO 常用发动机）          | (\hat A_t^{\text{GAE}(\gamma,\lambda)}=\sum_{l=0}^{\infty}(\gamma\lambda)^l,\delta_{t+l})，其中 (\delta_t=r_t+\gamma V(s_{t+1})-V(s_t)) ([Stanford University](https://web.stanford.edu/class/psych209/Readings/SuttonBartoIPRLBook2ndEd.pdf?utm_source=chatgpt.com "Reinforcement Learning: An Introduction - Stanford University"))                                                                   | 把 many-step TD 残差加权（(\lambda) 控制“更稳 vs 更准”） ([Stanford University](https://web.stanford.edu/class/psych209/Readings/SuttonBartoIPRLBook2ndEd.pdf?utm_source=chatgpt.com "Reinforcement Learning: An Introduction - Stanford University"))                  | advantage normalization、clip/KL ([arXiv](https://arxiv.org/abs/1707.06347?utm_source=chatgpt.com "Proximal Policy Optimization Algorithms"))                                                                           | PPO/AC 常用组件 ([arXiv](https://arxiv.org/abs/1707.06347?utm_source=chatgpt.com "Proximal Policy Optimization Algorithms"))                                                                                               |
| **Trust Region / PPO 近似信任域**（更新幅度控制）  | TRPO 约束形式：(\max_\theta \mathbb{E}!\left[\frac{\pi_\theta(a\mid s)}{\pi_{\theta_{\text{old}}}(a\mid s)},\hat A\right]\ \text{s.t. }\mathbb{E}[\mathrm{KL}(\pi_{\theta_{\text{old}}}\Vert\pi_\theta)]\le \delta) ([arXiv](https://arxiv.org/abs/1502.05477?utm_source=chatgpt.com "Trust Region Policy Optimization"))                                                                                 | ratio (\frac{\pi_\theta}{\pi_{\theta_{\text{old}}}}) + (\hat A)（优势估计器来自 GAE/TD） ([arXiv](https://arxiv.org/abs/1502.05477?utm_source=chatgpt.com "Trust Region Policy Optimization"))                                                                      | PPO 用 clipping 近似 trust region ([arXiv](https://arxiv.org/abs/1707.06347?utm_source=chatgpt.com "Proximal Policy Optimization Algorithms"))                                                                            | TRPO、PPO ([arXiv](https://arxiv.org/abs/1502.05477?utm_source=chatgpt.com "Trust Region Policy Optimization"))                                                                                                         |
| **Max-Entropy（最大熵 RL）**               | (\max_\pi\ \mathbb{E}!\left[\sum_{t\ge 0}\gamma^t\big(r_t+\alpha,\mathcal{H}(\pi(\cdot\mid s_t))\big)\right]) ([arXiv](https://arxiv.org/abs/1801.01290?utm_source=chatgpt.com "Soft Actor-Critic: Off-Policy Maximum Entropy Deep Reinforcement Learning with a Stochastic Actor"))                                                                                                                 | “软”价值备份（soft backup），把 (\log \pi) 或熵项写入 target ([arXiv](https://arxiv.org/abs/1801.01290?utm_source=chatgpt.com "Soft Actor-Critic: Off-Policy Maximum Entropy Deep Reinforcement Learning with a Stochastic Actor"))                                      | 双Q、target/Polyak、replay（SAC 常见实现） ([arXiv](https://arxiv.org/abs/1801.01290?utm_source=chatgpt.com "Soft Actor-Critic: Off-Policy Maximum Entropy Deep Reinforcement Learning with a Stochastic Actor"))               | SAC ([arXiv](https://arxiv.org/abs/1801.01290?utm_source=chatgpt.com "Soft Actor-Critic: Off-Policy Maximum Entropy Deep Reinforcement Learning with a Stochastic Actor"))                                             |
| **Off-policy 修正**（分布错配时）              | 重要性采样比率：(\rho_t=\frac{\pi(a_t\mid s_t)}{\mu(a_t\mid s_t)})；高吞吐场景用 V-trace 等修正 ([arXiv](https://arxiv.org/abs/1802.01561?utm_source=chatgpt.com "[1802.01561] IMPALA: Scalable Distributed Deep-RL with Importance ..."))                                                                                                                                                                             | IS / V-trace / Retrace（修正“行为策略 (\mu)”与“学习策略 (\pi)”不一致） ([arXiv](https://arxiv.org/abs/1802.01561?utm_source=chatgpt.com "[1802.01561] IMPALA: Scalable Distributed Deep-RL with Importance ..."))                                                          | 比率截断、稳定的多步 target ([arXiv](https://arxiv.org/abs/1802.01561?utm_source=chatgpt.com "[1802.01561] IMPALA: Scalable Distributed Deep-RL with Importance ..."))                                                           | IMPALA（V-trace） ([arXiv](https://arxiv.org/abs/1802.01561?utm_source=chatgpt.com "[1802.01561] IMPALA: Scalable Distributed Deep-RL with Importance ..."))                                                             |
| **Offline RL（保守/防 OOD）**              | 一种典型写法：(L_{\text{CQL}}=L_{\text{Bellman}}+\alpha!\left(\mathbb{E}_{s,,a\sim \pi(\cdot\mid s)}Q(s,a)-\mathbb{E}_{(s,a)\sim D}Q(s,a)\right)) ([arXiv](https://arxiv.org/abs/2006.04779?utm_source=chatgpt.com "Conservative Q-Learning for Offline Reinforcement Learning"))                                                                                                                           | 关键是“对数据外动作的 Q 更悲观”，抵抗分布偏移导致的过估计 ([arXiv](https://arxiv.org/abs/2006.04779?utm_source=chatgpt.com "Conservative Q-Learning for Offline Reinforcement Learning"))                                                                                            | 保守正则强度 (\alpha)、行为约束/混合 BC ([arXiv](https://arxiv.org/abs/2006.04779?utm_source=chatgpt.com "Conservative Q-Learning for Offline Reinforcement Learning"))                                                             | CQL 等离线算法 ([arXiv](https://arxiv.org/abs/2006.04779?utm_source=chatgpt.com "Conservative Q-Learning for Offline Reinforcement Learning"))                                                                              |
| **Model (\hat P,\hat r)**（模型派）        | 学动态：(\max_\psi\ \mathbb{E}_{(s,a,s')}!\left[\log \hat P_\psi(s'\mid s,a)\right])（或回归 (\min \|\hat s'-s'\|^2)） ([derongliu.org](https://www.derongliu.org/adp/adp-cdrom/refs/sutton19900216.pdf?utm_source=chatgpt.com "Integrated Architectures for Learning, Planning, and Reacting Based on ..."))                                                                                                 | one-step / short rollout（用模型生成短轨迹） ([derongliu.org](https://www.derongliu.org/adp/adp-cdrom/refs/sutton19900216.pdf?utm_source=chatgpt.com "Integrated Architectures for Learning, Planning, and Reacting Based on ..."))                                  | 短 rollout、ensemble、不确定性、MPC 重规划 ([derongliu.org](https://www.derongliu.org/adp/adp-cdrom/refs/sutton19900216.pdf?utm_source=chatgpt.com "Integrated Architectures for Learning, Planning, and Reacting Based on ...")) | Dyna ([derongliu.org](https://www.derongliu.org/adp/adp-cdrom/refs/sutton19900216.pdf?utm_source=chatgpt.com "Integrated Architectures for Learning, Planning, and Reacting Based on ..."))                            |
| **Distributional (Z^\pi)**（回报分布）      | 分布式贝尔曼：(Z^\pi(s,a)\overset{D}{=}r(s,a)+\gamma Z^\pi(s',a')) ([arXiv](https://arxiv.org/abs/1707.06887?utm_source=chatgpt.com "A Distributional Perspective on Reinforcement Learning"))                                                                                                                                                                                                              | 用分类/分位数回归等匹配分布（C51/QR-DQN） ([arXiv](https://arxiv.org/abs/1707.06887?utm_source=chatgpt.com "A Distributional Perspective on Reinforcement Learning"))                                                                                                     | 投影/分布匹配方式稳定训练 ([arXiv](https://arxiv.org/abs/1707.06887?utm_source=chatgpt.com "A Distributional Perspective on Reinforcement Learning"))                                                                              | Distributional RL 家族 ([arXiv](https://arxiv.org/abs/1707.06887?utm_source=chatgpt.com "A Distributional Perspective on Reinforcement Learning"))                                                                       |
| **Occupancy (\rho_\pi(s,a))**（占用度量视角） | (\rho_\pi(s,a)=(1-\gamma)\sum_{t=0}^{\infty}\gamma^t,\Pr_\pi(s_t=s,,a_t=a)) ([homes.di.unimi.it](https://homes.di.unimi.it/~cesabian/RL/Notes/discounted.pdf?utm_source=chatgpt.com "Discounted MDP - unimi.it"))                                                                                                                                                                                    | 把“优化策略”转成“优化占用分布”（常连到 LP/对偶/约束 RL） ([adityam.github.io](https://adityam.github.io/stochastic-control/mdps/linear-programming.html?utm_source=chatgpt.com "18 Linear programming formulation – Stochastic Control and Decision Theory"))                    | 拉格朗日、约束投影等 ([adityam.github.io](https://adityam.github.io/stochastic-control/mdps/linear-programming.html?utm_source=chatgpt.com "18 Linear programming formulation – Stochastic Control and Decision Theory"))        | 约束RL/IRL/GAIL视角常用 ([adityam.github.io](https://adityam.github.io/stochastic-control/mdps/linear-programming.html?utm_source=chatgpt.com "18 Linear programming formulation – Stochastic Control and Decision Theory")) |
| **Preference（非 RL rollout）**          | DPO 的典型目标：(\max_\theta \mathbb{E}\big[\log \sigma(\beta,\Delta)\big])，其中 (\Delta=\log\pi_\theta(y^+\mid x)-\log\pi_\theta(y^-\mid x)-\log\pi_{\text{ref}}(y^+\mid x)+\log\pi_{\text{ref}}(y^-\mid x)) ([arXiv](https://arxiv.org/abs/2305.18290?utm_source=chatgpt.com "Direct Preference Optimization: Your Language Model is Secretly a Reward Model"))                                            | 用偏好对比直接优化（避免在线 rollout） ([arXiv](https://arxiv.org/abs/2305.18290?utm_source=chatgpt.com "Direct Preference Optimization: Your Language Model is Secretly a Reward Model"))                                                                                | 训练更稳、实现更简单（但本质不是“在线 RL”） ([arXiv](https://arxiv.org/abs/2305.18290?utm_source=chatgpt.com "Direct Preference Optimization: Your Language Model is Secretly a Reward Model"))                                           | DPO ([arXiv](https://arxiv.org/abs/2305.18290?utm_source=chatgpt.com "Direct Preference Optimization: Your Language Model is Secretly a Reward Model"))                                                                |